...
anotherfile.welcome()
# after this we see "hey you are welcome from anotherfile" printed two times but we call for one time. But why it is being printed the second time around -> it is because we have done only import anotherfile and write also another welcome in 45fortyfive.py

# How to solve this problem?
# Write in anotherfile.py:
# if __name__ == "__main__":
#   welcome()
# I am basically saying that run it, if I run it from anotherfile.py then run otherwise can't run the welcome code from anotherfile.py
